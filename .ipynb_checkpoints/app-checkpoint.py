...
def charge_donnees_bourse(ticker):
    ...
    rendement = pd.DataFrame(index=[0])
    risque = pd.DataFrame(index=[0])
    rendement['MA50'] = year_vue['MA50'].iloc[-1]    
    rendement['Delta MA50'] = (year_vue['Close'].iloc[-1] / year_vue['MA50'].iloc[-1] - 1 if year_vue['MA50'].iloc[-1] != 0 else None)
    rendement['ATH / 1 an'] = year_vue['Close'].max()
    rendement['Delta ATH'] = (year_vue['Close'].iloc[-1] / year_vue['Close'].max() - 1 if year_vue['Close'].max() != 0 else None)
    rendement['Cours moyen / 1 an'] = year_vue['Close'].mean()
    rendement['Delta cours moyen'] = (year_vue['Close'].iloc[-1] / year_vue['Close'].mean() - 1 if year_vue['Close'].max() != 0 else None)
    rendement['Dividendes'] = year_vue['Dividends'].sum()
    rendement['Div %'] = rendement.apply(lambda row: row['Dividendes'] / row['Cours moyen / 1 an'] if row['Cours moyen / 1 an'] != 0 else None, axis=1)
    risque['Div %'] = rendement['Div %'].apply(div_risk_level)
    rendement['Rendement'] = (year_vue['Close'].iloc[-1] / year_vue['Close'].iloc[0] - 1 if year_vue['Close'].iloc[0] != 0 else None)
    risque['Rendement'] = rendement['Rendement'].apply(rdt_risk_level)
    rendements_jours = year_vue['Close'].pct_change().dropna()
    rendement['Volatilité'] = rendements_jours.std() * (252 ** 0.5)
    risque['Volatilité'] = rendement['Volatilité'].apply(vol_risk_level)
    rendement['Evol CA 0y'] = action_suivie.revenue_estimate.loc["0y", "growth"]
    rendement['Evol CA +1y'] = action_suivie.revenue_estimate.loc["+1y", "growth"]
    rendement['Evol EPS 0y'] = action_suivie.earnings_estimate.loc["0y", "growth"]
    rendement['Evol EPS +1y'] = action_suivie.earnings_estimate.loc["+1y", "growth"]
    rendement['Potentiel'] = action_suivie.analyst_price_targets['mean'] / action_suivie.analyst_price_targets['current'] -1
    return SimpleNamespace(action_suivie = action_suivie, day_vue = day_vue, year_vue = year_vue, rendement = rendement, risque = risque)
...
def plot_recommendations(action_suivie):
    ...
    colors = {"strongBuy": "#006400", "buy": "#32CD32", "hold": "#808080", "sell": "#FFA500", "strongSell": "#B22222"}
    ...
    for t in types:
        ...
        fig.add_trace(go.Bar(x=df_t["period"], y=df_t["count"], name=t, marker_color=colors[t]))
    fig.update_layout(template="plotly_dark",
                      height=250,
                      margin=dict(l=5, r=5, t=30, b=5),
...
if len(select_ticker.selection['rows']) == 0:
    ...
    # -- corps --
    with st.container():
        # -- Affichage données financières et cours du jour
        with st.container(height=620):
            col1, col2 = st.columns([0.6, 0.4], gap="small", border=True)
            # -- Données financières dans des onglets
            tab1, tab2, tab3 = col1.tabs(["Données financières", "Informations société", "Définitions"])
            ...
            with tab1.container(gap="xsmall", border=False, width="stretch", height=500):
                with st.container(horizontal=True, border=True, width="stretch"):
                    ...
                    m2.metric('Cours moyen / 1 an', data.rendement['Cours moyen / 1 an'], f"{data.rendement['Delta cours moyen'].iloc[0] * 100:.2f} %", format="euro")
                    m3.metric('MA50', data.rendement['MA50'], f"{data.rendement['Delta MA50'].iloc[0] * 100:.2f} %", format="euro")
                    m4.metric('ATH / 1 an', data.rendement['ATH / 1 an'], f"{data.rendement['Delta ATH'].iloc[0] * 100:.2f} %", format="euro")
                with st.container(horizontal=True, border=True, width="stretch"):
                    m5, m6, m7, m8 = st.columns(4, width="stretch")
                    m5.metric('Dividendes', data.rendement['Dividendes'], format="euro")
                    m6.metric('Div %', f"{data.rendement['Div %'].iloc[0] * 100:.2f} %", icon=data.risque['Div %'].iloc[0])
                    m7.metric('Rendement', f"{data.rendement['Rendement'].iloc[0] * 100:.2f} %", icon=data.risque['Rendement'].iloc[0])
                    m8.metric('Volatilité', f"{data.rendement['Volatilité'].iloc[0] * 100:.2f} %", icon=data.risque['Volatilité'].iloc[0])
                with st.container(horizontal=True, border=True, width="stretch"):
                    try:
                        fig_obj = plot_objectif(data.action_suivie)
                        st.plotly_chart(fig_obj, width="stretch", height=200, config={"displayModeBar": False})
                    except:
                        st.write("**Objectifs de cours**")
                        st.write("Données indisponibles")
                    try:
                        fig_reco = plot_recommendations(data.action_suivie)
                        st.plotly_chart(fig_reco, width="stretch", height=200, config={"displayModeBar": False})
                    except:
                        st.write("**Recommandations des analystes**")
                        st.write("Données recommandations indisponibles")
            ...
            # -- Graphique du cours du jour
            with col2.container(vertical_alignment="center"):
                try:
                    fig_day = plot_intraday(data.day_vue, data.action_suivie)
                    st.plotly_chart(fig_day, width=500, height="content")
                except:
                    st.write("Données indisponibles")
        # -- Graphique de l'évolution à 12 mois
